...
const compareCards = (card) => {
    ...
    let currentData = card.getAttribute("data-card");
    //booleans-variabler för jämförelse
    const noClickIsSet = !click1 && !click2;
    const hasClick1ButNot2 = click1 && !click2;
    const cardIsSame = card === (click1 === null || click1 === void 0 ? void 0 : click1.card);
    //sätter click1
    if (noClickIsSet) {
    ...
    const hasBothClickedCards = click1 && click2;
    if (hasBothClickedCards) {
        const isNoMatch = click1.data !== click2.data;
        if (isNoMatch) {
    ...
    const allCardHasFlip = cardElArray.every((card) => card.classList.contains("flip"));
    if (allCardHasFlip) {
        winTheGame();
    }
};
